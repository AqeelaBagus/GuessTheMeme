...
def calculate_score(master_data, user_landmarks):
    """
    master_data: numpy array shape (33, 4) [x, y, z, visibility]
    user_landmarks: list of dicts [{x, y, z, visibility}, ...]
    """
    if user_landmarks is None or master_data is None:
        return 0

    # Konversi user_landmarks dari list of dict ke list of list
...
def check_score():
    """
    Terima landmark dari browser (MediaPipe.js), hitung skor di server.
    Body JSON: { landmarks: [{x, y, z, visibility}, ...] }
    """
    data = request.get_json()
    landmarks = data.get('landmarks') if data else None
    final_score = calculate_score(game.master_data, landmarks)
